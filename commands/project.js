...
async function handleCreate(interaction) {
  ...
  try {
    ...
    // 3-1. goals 포럼 채널 자동 생성
    let forumMention = '';
    try {
      const { createForumChannel, FORUM_CHANNEL_NAME } = require('../src/forum');
      // createForumChannel은 이미 존재하면 기존 채널을 반환하므로,
      // 호출 전에 존재 여부를 미리 확인하여 created 여부를 판단합니다.
      const { ChannelType: CT } = require('discord.js');
      const alreadyExists = guild.channels.cache.some(
        (c) => c.type === CT.GuildForum && c.name === FORUM_CHANNEL_NAME,
      );
      const forumCh = await createForumChannel(guild, { categoryId: category.id });
      if (alreadyExists) {
        log.push(`  🗂️  <#${forumCh.id}> goals 포럼 채널 — 이미 존재, 스킵`);
        forumMention = `\nℹ️ <#${forumCh.id}> goals 포럼 채널이 이미 존재합니다.`;
      } else {
        log.push(`  🗂️  <#${forumCh.id}> goals 포럼 채널 — 생성 완료`);
        forumMention = `\n✅ <#${forumCh.id}> goals 포럼 채널이 함께 생성되었습니다.`;
      }
    } catch (forumErr) {
      console.error('[/project create] goals 포럼 채널 생성 실패:', forumErr);
      log.push('  ⚠️  goals 포럼 채널 생성 중 오류 발생');
    }
