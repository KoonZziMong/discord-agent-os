...
### 역할 컨텍스트 로딩 구조 (Step 0 ~ 3)

봇은 응답 시 아래 순서로 역할 컨텍스트를 누적 로드합니다.

```
Step 0 │ ROLE 카테고리 'rule' 채널 (팀 공통 규약)
       │ 역할·채널 관계없이 모든 에이전트에 항상 주입
       │ → 약한/강한 결합 인터페이스, 에스컬레이션 규칙 등 정의
       │
Step 1 │ ROLE 카테고리 역할 채널 (role/developer 등)
...
2. 역할별 채널 생성: `rule`, `orchestrator`, `planner`, `developer`, `reviewer`, `tester`, `researcher`
3. 각 채널에 기본 역할 내용 핀 등록
4. 각 채널에 CmdBot 디폴트 봇 핀 등록 (초기값: 미설정, `rule` 채널 제외)
...
**주요 동작:**
- 채널이 없으면 새로 생성 (`/role init` 이후 코드에 추가된 역할 채널 적용)
- 기존 역할 내용 핀 언핀 (메시지는 채널에 남아 히스토리 보존)
- CmdBot 디폴트 봇 핀은 유지 (수동 설정 보존)
...
>
> 💡 `/role init` 이후 코드 업데이트로 새 역할 채널(예: `rule`)이 추가된 경우 `/role reset`으로 누락 채널을 생성할 수 있습니다.
...
3. `workspace` 채널 생성
4. `goals` 포럼 채널 자동 생성 (이미 존재하는 경우 스킵 후 안내 메시지 출력)
5. `default_role` 활성화 시:
...
```
✅ 내앱 프로젝트 생성 완료
📁 카테고리: 내앱 | 💬 채널: #role, #workspace, #goals | 📌 디폴트 봇 매핑 완료 | 🤖 LLM 지침 적용
```
...
- **위치:** 프로젝트 카테고리 내 `goals` 포럼 채널
- **목적:** 목표 단위 추적 및 작업 문서화

**게시물(Thread) 구조:**
- 제목: `[goalId] 목표 요약`
- 본문: 목표 상세 내용, 수행 일시, 요청자
...
**진행 기록:**
- 오케스트레이터가 각 태스크 시작·완료·실패 시 thread에 자동 append

**최종 결과:**
- 목표 완료 시 전체 태스크 결과 요약을 thread에 기재
...
```
1. /role init
   → role 카테고리/채널 생성 (rule, orchestrator, planner, developer, reviewer, tester, researcher)
   → 이미 init 했다면 /role reset 으로 누락 채널(rule 등) 생성 가능
